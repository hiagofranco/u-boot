...
/* Only ports 0 to 2 are configured by ddrc_init() */
#define DDRC_NUM_PORTS			3
...
{
	u32 rddata, pwrctl, rtt_wr = 0, rtt_nom = 0;
	bool wr_odt_en;
 ...
 */
int ctrl_init_detect_dram_size(u8 *dram_cap_in_mbyte)
{
	...
	ret = ddr_readl_poll(DDR_BIST_BASE + BIST_STATUS, rddata, rddata & BIST_STATUS_DONE);
	writel(0x00010000, DDR_BIST_BASE + BIST_CTRL);

	while (!ret && cap_in_mbyte < 15) {
		cap_in_mbyte++;
		...
		writel(0x00010001, DDR_BIST_BASE + BIST_CTRL);
		ret = ddr_readl_poll(DDR_BIST_BASE + BIST_STATUS, rddata,
				     rddata & BIST_STATUS_DONE);
		writel(0x00010000, DDR_BIST_BASE + BIST_CTRL);
		if (ret)
			break;
		...
		ret = ddr_readl_poll(DDR_BIST_BASE + BIST_STATUS, rddata,
				     rddata & BIST_STATUS_DONE);
		writel(0x00010000, DDR_BIST_BASE + BIST_CTRL);
		if (!ret && (rddata & BIST_STATUS_FAIL))
			break;
	}
	...
	/* Without a corrupted background pattern the size is unknown */
	if (ret || !(rddata & BIST_STATUS_FAIL)) {
		pr_err("DDR: size detection failed\n");
		return -ENODEV;
	}
